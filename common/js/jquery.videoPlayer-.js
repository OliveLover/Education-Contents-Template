;(function($){
	...
	$.fn.videoPlayer = function(options){
		...
		return this.each(function(){
			...
			if(cur_app != "none") $("#fullscreen").attr("disabled",true).css({"opacity":0.5,"cursor":"default"});
			...
			$rate_btn.on({
				mouseover : function(){
					$(this).css({"color":"#FDA947","font-family":"scd6"});
				},
				mouseout : function(){
					var _rate = parseInt($(this).attr('id').replace("rate-",""),10);
					if(_rate != curRate){
						$(this).css({"color":"#fff","font-family":"scd2"});
					}
				},
				click : function(){
					var _rate = parseInt($(this).attr('id').replace("rate-",""),10);
			...
			function mouseOver_fn(_id){
				if(_id == "volume"){
					if(vp.muted) $("#"+_id).css({"background-position-x":"-"+$control_btn.width()+"px"});
				}else{
					if(_id != "play") $("#"+_id).css({"background-position-x":"-"+$control_btn.width()+"px"});
				}
			}

			function mouseOut_fn(_id){
				if(_id == "caption"){
					if(captionToggle == "close") $("#"+_id).css({"background-position-x":"0px"});
				}else if(_id == "volume"){
					if(vp.muted) $("#"+_id).css({"background-position-x":"0px"});
				}else{
					if(_id != "play") $("#"+_id).css({"background-position-x":"0px"});
				}
			}
			...
			function select_rate(n){
				...
				$rate_btn.css({"color":"#999","font-family":"scd2"});
			...
			function playToggle(){
				if(!vp.paused){
					$play_btn.css({"background-position-x":"0px"});
					if(vp.ended){
						$play_btn.css({"background-position-x":"-"+$play_btn.width()+"px"});
					}
				}else{
					$play_btn.css({"background-position-x":"-"+$play_btn.width()+"px"});
				}
			...
			function setVolume(){
				...
				$volume_btn.css({"background-position-x":"-"+$volume_btn.width()+"px"});
			}
			...
			function muteVolume(){
				if(vp.muted){
					...
					$volume_btn.css({"background-position-x":"0px"});
				}
			}
