; (function ($) {
	...
	$.fn.videoPlayer = function (options) {
		...
		return this.each(function () {
			...
			controls_html += '<button class="control-btn" id="download"></button>';
			...
			$rate_btn.on({
				...
				click: function () {
					var _rate = parseInt($(this).attr('id').replace("rate-", ""), 10);
					if (portingMode == true && learnedChk == false) {
						alert('학습을 완료한 후 배속 변경이 가능합니다.');
					} else {
						select_rate(_rate);
					}
				}
			...
			function mouseOver_fn(_id) {
				if (_id == "volume") {
					...
					$("#" + _id).css({ "background-position-x": "-" + $control_btn.width() + "px" });
				}
			}
