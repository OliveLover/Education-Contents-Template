...
flowInfo = [
	{ "code": "preview", "index": "Preview" },
	{ "code": "lesson", "index": "Lesson" },
	{ "code": "review", "index": "Review" },
	{ "code": "fine", "index": "Fine" }
]
flowInfo["preview"] = []
flowInfo["lesson"] = []
flowInfo["review"] = []
...
function buildIndex() {
	...
	$(".flow-index").on({
		mouseover: function () {
			$(this).children(".flow-bullet").css({ "background-position-x": "-" + $(".flow-bullet").width() + "px" });
		...
		click: function () {
			...
			if (now_page < _page && portingMode == true && learnChk == null) {
				alert("순차학습하세요.");
			} else {
				movePage(_page);
			}
